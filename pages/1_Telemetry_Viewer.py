...
# === TOP BAR: Track + Run type + Upload (no direct session_state assignment) ===
st.markdown("### ")
top = st.container()
with top:
    ...
    track_pick = c1.selectbox("Track", track_names, index=default_idx, key="track_pick")
    run_type   = c2.radio("Run type", ["Practice","Qualifying","Race"], index=0, horizontal=True, key="run_type")
    up         = c3.file_uploader("Upload telemetry (.csv or .ibt)", type=["csv","ibt"], key="up_file")

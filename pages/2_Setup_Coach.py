...
def ensure_allowed(d):
    ...
    allow_flat = {p for plist in ALLOWED.values() for p in plist}
...
for i, meta in enumerate(corner_meta):
    ...
    with cols[i % 3]:
        ...
        st.markdown(
            f"**{c}**  \n<small>Dir: {dlabel} • Bank: {meta.get('bank_deg',0):g}° • Angle: {meta.get('angle_deg',90):g}°</small>",
...
if btn:
    ...
    for k, v in tblock.items():
        plan[k].extend(v)
    if tscale > 0:
        findings.append("Temperature: " + str(abs(tdiff)) + "°F " + ("hotter" if tdiff>0 else "cooler") + f" than baseline (x{tscale})")
    ...
    for meta in corner_meta:
        ...
        findings.append(f"{name} ({side}; bank {meta.get('bank_deg',0):g}°, angle {meta.get('angle_deg',90):g}°): {fb['feels']} ({sev})")
